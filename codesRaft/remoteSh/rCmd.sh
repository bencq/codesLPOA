...
do
    ...
    if [ "${cmd}" == "copy" ]; then 
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t node 'PRIVATE_CONFIG=ignore ./geth --datadir data --port ${PORT} --nodiscover --syncmode full --networkid ${NETWORKID} --raft --raftport ${RAFTPORT} --raftblocktime 1000 --emitcheckpoints --ws --ws.addr 0.0.0.0 --ws.api admin,db,eth,debug,miner,net,shh,txpool,personal,web3,quorum,raft --ws.origins \"*\" --emitcheckpoints --allow-insecure-unlock --verbosity ${verbosity} &>test${i}.log' C-m"
    ...
    #     break
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux new -s con -d"
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t con 'eth.sendTransaction({from: eth.coinbase, to:eth.coinbase, value:100}, \"\")' C-m"
