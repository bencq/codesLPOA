...
do
    ...
    if [ "${cmd}" == "copy" ]; then 
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t node './geth --datadir data --port ${PORT} --nodiscover --syncmode full --networkid 666667 --ws --ws.addr 0.0.0.0 --ws.api web3,eth --ws.origins \"*\" --allow-insecure-unlock --verbosity ${verbosity} &>test${i}.log' C-m"
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t con './geth --datadir data attach' C-m"
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux new -s con -d"
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t con 'personal.unlockAccount(eth.coinbase, \"\", 0)' C-m"
        ...
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux new -s con -d"
        sshpass -p "${pswd}" ssh -n -o StrictHostKeyChecking=no ${userHost} "tmux send-keys -t con 'personal.sendTransaction({from: eth.coinbase, to:eth.coinbase, value:100}, \"\")' C-m"
